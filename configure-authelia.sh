...
log "Auth Portal: $AUTH_SUBDOMAIN"
...
session:
  ...
  cookies:
    - domain: '$PARENT_DOMAIN'
      authelia_url: 'https://$AUTH_SUBDOMAIN'
...
# ============================================
# Step 8: Expand SSL Certificate
# ============================================
log "Step 8: Checking SSL certificate for auth subdomain..."
...
# Check if auth subdomain is already in certificate
if sudo certbot certificates 2>/dev/null | grep -q "$AUTH_SUBDOMAIN"; then
    log "Auth subdomain already in certificate"
else
    log "Expanding certificate to include auth subdomain..."
    warn "Make sure $AUTH_SUBDOMAIN DNS record points to this server!"
    echo -n "Press Enter to continue or Ctrl+C to abort..."
    read -r

    # Expand certificate using webroot method
    sudo certbot certonly --expand --webroot -w /var/www/certbot \
        -d "$CERT_DOMAIN" -d "$AUTH_SUBDOMAIN" \
...
# Create new Nginx configuration with Authelia
sudo tee /etc/nginx/sites-available/wazuh-authelia > /dev/null <<EOF
# Authelia + Wazuh Dashboard Configuration
# Generated by configure-authelia.sh
...
server {
    listen 80;
    server_name $DOMAIN_NAME $AUTH_SUBDOMAIN;
...
# Wazuh Dashboard with Authelia Protection
server {
    listen 443 ssl http2;
    ...
    # Authelia internal endpoint for forward auth
    location /internal/authelia/authz {
        internal;

        proxy_pass http://authelia/api/authz/forward-auth;
    ...
    location / {
        ...
        error_page 401 =302 https://$AUTH_SUBDOMAIN/?rd=\$scheme://\$http_host\$request_uri;
...
echo "Authelia Portal: https://$AUTH_SUBDOMAIN"
echo ""
...
echo "2. You will be redirected to https://$AUTH_SUBDOMAIN"
...
warn "IMPORTANT: Ensure $AUTH_SUBDOMAIN DNS record points to $SERVER_IP"
